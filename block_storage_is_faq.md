...
For information on how you are billed, see [Pricing for Block Storage for VPC](/docs/vpc-on-classic?topic=vpc-on-classic-block-storage-pricing).
Certain limits apply.  For more information about quotas and limits for your {{site.data.keyword.cloud}} Virtual Private Cloud and the resources available within it, see [Quotas](/docs/vpc-on-classic?topic=vpc-on-classic-quotas#quotas).

## After creating a new volume, can I increase its capacity later?
...
You do not have to pre-warm a volume. You will see the specified throughput immediately upon provisioning the volume.
...
Block Storage for VPC secures your data across redundant fault zones in your region.  We also encourage you to independently backup your data. You might also consider using  [Block Storage for IBM Cloud](/docs/BlockStorage/index.html#getting-started-with-block-storage), which provides disaster recovery features such as volume cloning, snapshots, and replication.
...
## Will firewalls or security groups impact performance?

As best practice, we recommend running storage traffic on a VLAN that bypasses the firewall. Running storage traffic through software firewalls increases latency and adversely affects storage performance.

## When can I delete a block storage data volume?

You can delete a block storage data volume only when it's not attached to a virtual server instance. [Detach the volume](/docs/vpc-on-classic-block-storage?topic=vpc-on-classic-block-storage-managing-block-storage#detach)
before deleting it.  Boot volumes are detached and deleted when the VSI is deleted.
...
Instead, use an IBM Cloud data classic service option such as Cloud Object Storage or Couldant if you must share your data across multiple zones and regions.
